...
## Git コマンドの実行について
コミットメッセージを作成するために git show や git diff などのページャーが必要になる可能性があるコマンドを使うとき、これらの出力が長い場合にページャーが起動してしまい、対話的な操作が必要になることがあります。そのため、ページャーが必要になる可能性があるコマンドを実行する際は、以下のようにcatにパイプして出力を一度に表示する必要がある (SHOULD)。
